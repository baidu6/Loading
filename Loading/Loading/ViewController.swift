...
class ViewController: UIViewController {
    ...
    fileprivate var titlesArr:[String] = ["登录捂眼动画","进度条动画","Loading动画","登录炫酷动画","加入购物车","签到动画","导航波纹","数字金额变化","鼓掌动画","手势密码解锁","转场动画--小圆点扩散","转场动画---弹性pop","网上学习布局方式练习","可以点击的饼图","导航波纹","IOS和H5手势交互","粒子点赞效果","仿UC头部下拉动画","数字上下滑动动画效果","高端理财","tableView点击状态的切换效果","牛逼的侧滑效果","长按移动标签","导航双波浪动画波纹","Menu动画","mask使用。渐变进度条","UILabel的缩放动画","导航头像缩放（随着滑动）","tableView的展开缩放动画","shining动画","点击tableView的cell的展开动画","searchBar的练习使用","YH弹窗的练习","swift简单的加载动画效果","tableView插入cell"]
...
extension ViewController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0{
            ...
            let nav = UINavigationController(rootViewController: InsertDatasViewController())
            self.present(nav, animated: true, completion: nil)
        }
    }
}
